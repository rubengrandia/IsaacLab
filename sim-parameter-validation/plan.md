...
(`JOINT-09`) are now implemented upstream. Kamino restitution (`MAT-04`) also passes after
[newton-physics/newton#3779](https://github.com/newton-physics/newton/pull/3779) added speculative-contact
culling; its fixture now detects the first post-impact trajectory minimum instead of relying on an end-of-step
contact-force sample that can miss impacts occurring within solver substeps. The matrix promotes all three
Newton-Kamino rows from `X` to `I`, and the stale strict expected-failure marks were removed.

One complete run on an NVIDIA GeForce RTX 5090 collected 372 tests and produced **354 passed and 18 expected
failures** once the stale Kamino marks were cleared. The remaining 18 expected failures are:

- `MAT-04`: `test_mat_04_restitution_first_rebound[mjwarp-{usd,cfg,runtime}]` (3 cases).
...
| SHAPE-03 | Collision radius | T / T / N | I / I / N | I / I / N |
...
| MAT-04 | Restitution | T / T / T | X / X / X | I / I / I |
...
- **Ground collider:** use a static collider with no rigid-body or mass API, matching the normal Newton scene
  topology. Runtime material and offset event terms require a rigid asset, so the static ground is authored with
  the target value while the public runtime event updates only the dynamic object; both contacting shapes have
  the same target value when the measurement begins.
...
- **Restitution:** drop a sphere from height \(h_0\) and compare its first rebound height with
  ...
  restitution model. Detect the first post-impact minimum from the position or normal-velocity trajectory; an
  end-of-step contact-force sample can miss an impact that rebounds within solver substeps. Kamino passes this
  fixture with the default positive contact offset after speculative-contact culling was added in
  [newton-physics/newton#3779](https://github.com/newton-physics/newton/pull/3779).
...
| FIX-RESTITUTION | MAT-04 | First post-impact apex or normal-velocity ratio | Static ground collider, zero friction; first impact only; detect impact from the trajectory rather than an end-of-step force sample |
| FIX-SHAPE-CONTACT | SHAPE-01, SHAPE-02, SHAPE-03 | First-contact step and resting separation relative to a control shape | Static ground collider; slow approach; fixed dynamic-body pose and velocity |
| FIX-CONTACT-OFFSET | CONTACT-01 | First generated-contact step and equilibrium separation | Static ground collider; paired baseline; report public rest/contact offsets and mapped margin/gap |
...
3. **Phase 2 — contact parameters (Newton only, complete with accepted gaps):** friction, restitution, geometry,
   and offset fixtures are implemented for Newton-MJWarp and Newton-Kamino. Strict expected failures remain for
   MJWarp restitution and contact margins on MJWarp.
...
Newton shape margins. The fixture now uses a static ground collider instead of a kinematic rigid body. With that
topology, Kamino implements all authoring paths for `MAT-03` and `CONTACT-01`, both authoring paths for
`SHAPE-01`, `SHAPE-02`, and `SHAPE-03`, and the `SHAPE-02` runtime error contract. Kamino also implements all
three `MAT-04` authoring paths. Its first rebound reaches `0.6506` m for the `0.6400` m oracle with the fixture's
default `contact_offset = 0.01` m. The detecting test uses the first downward-to-upward normal-velocity
transition because contact-force reporting occurs after all solver substeps and can miss the transient impact.
...
| Kamino restitution positive-gap defect | Newton/Kamino integration maintainer | Resolved for v1 by [newton-physics/newton#3779](https://github.com/newton-physics/newton/pull/3779); Kamino `FIX-RESTITUTION` passes the rebound-height oracle on all three authoring paths |
