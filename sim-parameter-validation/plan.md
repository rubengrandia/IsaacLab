...
- **Restitution:** drop a sphere from height \(h_0\) and compare its first rebound height with
  ...
  restitution model. Kamino currently fails this fixture when either shape carries a positive contact gap because
  speculative stabilization and restitution biases combine before penetration
  ([vastsoun/newton#375](https://github.com/vastsoun/newton/issues/375)); upstream zero-gap evidence is in
  [newton-physics/newton#3588](https://github.com/newton-physics/newton/pull/3588).
...
| FIX-RESTITUTION | MAT-04 | First post-impact apex or normal-velocity ratio | Static ground collider, zero friction; first impact only; after [vastsoun/newton#375](https://github.com/vastsoun/newton/issues/375) is fixed, rebound must be gap-invariant across margin/gap sweeps |
...
`SHAPE-01`, `SHAPE-02`, and `SHAPE-03`, and the `SHAPE-02` runtime error contract. Kamino `MAT-04` remains a
strict expected failure because positive-gap contacts combine stabilization and restitution biases while still
