...
def _gather_base_state_from_joints(
    ...
    reconcile and is therefore stale at this point. (Sourcing from ``body_q`` would also
    double-transform fixed bases, whose ``base_q`` is relative to the reference pose.)

    Non-free base joints (e.g. a fixed base) carry no base pose in the joint state, so the
    output defaults to identity pose / zero twist; the FK solver then keeps the base at its
    reference pose. Worlds without a base joint (``base_joint_index < 0``) are likewise left
    at the default.
    """
    wid = wp.tid()
...
class NewtonKaminoManager(NewtonManager):
    ...
    def _forward_kamino(cls, world_mask: wp.array | None = None) -> None:
        ...
        free joint's ``joint_q[0:7]`` / ``joint_qd[0:6]`` head (floating bases). Without it, the
        FK solver falls back to a single constant default base pose for every world. Fixed
        bases carry no base pose in the joint state and keep the FK reference pose.
        ...
        # state (where a floating base's root pose/velocity write lands). Reading ``joint_q`` /
        # ``joint_qd`` -- not ``body_q`` -- is required because ``body_q`` is only refreshed by
        # this reconcile and is stale here.
        wp.launch(
        ...
        # the FK solution. Joint coordinates are derived from relative body orientations, which
        # are frame-invariant, so the body-origin-frame poses in ``state_0`` give correct values
        # for revolute joints (the orientation-based coordinate is unaffected by COM offsets).
