...
@pytest.mark.xfail(
    reason="Split write path passes global_torque_w (about world origin) directly to PhysX. "
    "The positional torque correction only happens in compose_to_body_frame, which is the Newton path. "
    "Global forces with positions need compose_to_body_frame or position_data passed to PhysX.",
)
@pytest.mark.parametrize("device", ["cuda:0", "cpu"])
