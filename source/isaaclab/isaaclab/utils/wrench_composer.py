...
class WrenchComposer:
    def __init__(self, asset: BaseArticulation | BaseRigidObject | BaseRigidObjectCollection) -> None:
        ...
        The dual-buffer architecture uses five input buffers:
            ...
            - global_force_at_com_w: Global forces applied at the center of mass (world frame).
    ...
    def global_only(self) -> bool:
        """Whether only global-frame forces/torques have been applied (no local contributions)."""
    ...
    def global_force_at_com_w(self) -> wp.array:
        """Global force at CoM buffer (world frame). Shape: (num_envs, num_bodies, 3)."""
    ...
    def add_forces_and_torques_index(
        ...
        Args:
            ...
            positions: Positions (offsets from CoM). Shape: (len(env_ids), len(body_ids), 3). Defaults to None.
    ...
    def set_forces_and_torques_index(
        ...
        self._active = True
        self._dirty = True
        # set replaces everything, so flags match is_global exactly
        self._has_global = is_global
        self._has_local = not is_global
    ...
    def add_forces_and_torques_mask(
        ...
        Args:
            ...
            positions: Positions (offsets from CoM). Shape: (num_envs, num_bodies, 3). Defaults to None.
    ...
    def reset(self, env_ids: wp.array | torch.Tensor | None = None, env_mask: wp.array | None = None):
        ...
        if env_ids is None and env_mask is None:
            ...
            # Partial reset via mask: 7 buffers = 3.5 pairs, call 4 times
            # Pair 1: global_force_w, global_torque_w
            wp.launch(
                reset_wrench_composer_mask,
                dim=(self.num_envs, self.num_bodies),
                inputs=[
            ...
            # Pair 2: global_force_at_com_w, local_force_b
            wp.launch(
                reset_wrench_composer_mask,
                dim=(self.num_envs, self.num_bodies),
                inputs=[
            ...
            # Pair 3: local_torque_b, out_force_b
            wp.launch(
                reset_wrench_composer_mask,
                dim=(self.num_envs, self.num_bodies),
                inputs=[
            ...
            # Pair 4: out_torque_b, out_torque_b (zeroing same buffer twice is fine)
            wp.launch(
                reset_wrench_composer_mask,
                dim=(self.num_envs, self.num_bodies),
                inputs=[
